...
for chiUnderline in 0.2 0.5 1.0
do
    for a_e in 0.15
    do
        for a_h in 0.1 0.05
                ...
                do
                    ...
                    do
                        ...
                        do
                            ...
                            touch ./bash/$domain_folder/$model_folder/preold.sh
                            tee ./bash/$domain_folder/$model_folder/preold.sh << EOF
#! /bin/bash

#SBATCH --account=pi-lhansen
#SBATCH --job-name=preold
#SBATCH --output=./job-outs/$domain_folder/$model_folder/preold.out
#SBATCH --error=./job-outs/$domain_folder/$model_folder/preold.err
...
python3 /project/lhansen/mfrSuite_Ela_midway3/SolvedModels/oldrun.py --chiUnderline ${chiUnderline} --a_e ${a_e} --a_h ${a_h} --gamma_e ${gamma_e} --gamma_h ${gamma_h} --psi_e ${psi_e} --psi_h ${psi_h} \
                                                    --nV ${nV} --nVtilde ${nVtilde} --V_bar ${V_bar} --Vtilde_bar ${Vtilde_bar} --sigma_V_norm ${sigma_V_norm} --sigma_Vtilde_norm ${sigma_Vtilde_norm} \

EOF
                            sbatch ./bash/$domain_folder/$model_folder/preold.sh
                        done
                    done
                done
            done
